...
app = FastAPI(title="ContrastCheck API", version="1.1.0")
...
class CutoutRequest(BaseModel):
    ...
    border_pct: float = 0.12  # 12% of min(width,height) used as garment ring
    thresholds: Optional[Thresholds] = Thresholds()

# ---------- Core: box-mode (kept for compatibility) ----------
...
def contrastcheck(req: RequestBoxMode):
    ...
    for loc in req.locations:
        ...
        verdict = "pass"
        if failing_garment_pairs or intra_fail: verdict = "fail"
        elif any(p["borderline"] for p in g_vs_d): verdict = "warn"
        notes = []
...
def split_border_inner(arr: np.ndarray, border_pct: float) -> Tuple[np.ndarray, np.ndarray]:
    """Return (border_pixels, inner_pixels) from an RGB array."""
    H, W, _ = arr.shape
    t = max(1, int(round(border_pct * min(W, H))))
    if t*2 >= min(W, H):
        # If the cutout is tiny, just use entire image as garment and design (fallback)
...
def contrastcheck_cutout(req: CutoutRequest):
    ...
    # garment from border, design from inner
